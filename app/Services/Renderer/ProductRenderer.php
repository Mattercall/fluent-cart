...
class ProductRenderer
{
    ...
    public function renderPromotionalSection()
    {
        ...
        // Backward compatibility for old single-section settings.
        if (isset($settings['image']) || isset($settings['heading']) || isset($settings['description'])) {
            $settings = [$settings];
        }

        $sections = array_filter($settings, function ($section) {
            if (!is_array($section)) {
                return false;
            }
            ...
            $heading = sanitize_text_field(Arr::get($section, 'heading', ''));
            $description = sanitize_textarea_field(Arr::get($section, 'description', ''));

            return !(empty($imageUrl) && empty($heading) && empty($description));
        });
